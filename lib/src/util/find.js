...
const projectHasFile = (...pathArgs) =>
  fs.existsSync(rootRelativePath(...pathArgs));
...
const eslintConfig = findConfig('eslint config', '.eslintrc', 'eslintConfig', [
  '.eslintrc',
  '.eslintrc.json',
  '.eslintrc.yml',
  '.eslintrc.yaml'
...
const eslintIgnore = findConfig('eslint ignore', '.eslintignore', null, [
  '.eslintignore'
]);
...
const prettierIgnore = findConfig('prettier ignore', '.prettierignore', null, [
  '.prettierignore'
]);

const babelConfig = findConfig('babel config', 'babel.config.js', 'babel', [
  'babel.config.js',
  '.babelrc',
  '.babelrc.js'
]);

const webpackConfig = findConfig('webpack config', 'webpack.config.js', null, [
  'webpack.config.js'
]);
